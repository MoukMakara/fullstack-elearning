...
function AdminClassroomPage() {
  ...
  const onClickBtnEdit = (item, index) => {
    ...
    if (item.StartTime && item.EndTime) {
      TimeDuration = [
        dayjs("0000-00-00 " + item.StartTime),
        dayjs("0000-00-00 " + item.EndTime),
      ];
    }
  ...
  return (
    <MainAdminPage loading={loading}>
      ...
      <Table
        ...
        columns={[
          ...
          },
          {
            ...
            title: "Due",
            render: (value, items) =>
              Number(items.TotalToPay) - Number(items.TotalPaid),
          },
          {
            key: "Time",
            title: "Time",
            render: (value, items) =>
              formatTimeClient(items.StartTime) +
              "-" +
              formatTimeClient(items.EndTime),
          },
          {
            key: "Date",
            title: "Date",
            render: (value, items) =>
              formatDateClient(items.StartDate) +
              "-" +
              formatDateClient(items.EndDate),
          },
          // {
          //   key: "IsActive",
          //   title: "IsActive",
          ...
          // },
          {
            ...
            render: (value, items, index) => (
              <Space>
                <Button
                  onClick={() => onClickBtnEdit(items, index)}
                  type="primary"
                  icon={<MdEdit />}
                />
                <Button
                  onClick={() => onClickBtnDelete(items, index)}
                  type="primary"
                  icon={<MdDelete />}
                  danger
                />
                <Button
                  onClick={() => onViewMore(items, index)}
                  type="primary"
                  icon={<MdMoreHoriz />}
                />
              </Space>
      ...
      <Modal
        ...
        title={formRef.getFieldValue("Id") ? "Edit Classroom" : "New Classroom"}
        footer={null}
      >
          ...
          <Row gutter={16}>
            <Col span={12}>
              ...
              >
                <Select
                  ...
                  disabled={formRef.getFieldValue("Id")}
                >
              ...
              <Form.Item label="Course GN" name="CourseGeneration">
                <InputNumber
                  ...
                  disabled
                  style={{ width: "100%" }}
                  placeholder="Course Genderation"
                />
              </Form.Item>
              ...
              <Form.Item
                ...
                name="LearningType"
                rules={[
                  { required: "true", message: "Please select Learning Type!" },
                ]}
              >
            ...
            <Col span={12}>
              ...
              <Form.Item
                ...
                name="ClassShiff"
                rules={[
                  { required: "true", message: "Please select Class shiff!" },
                ]}
              >
              ...
              <Form.Item
                ...
                name="IsActive"
                rules={[
                  { required: "true", message: "Please fill in status!" },
                ]}
              >
                <Select
                  style={{ width: "100%" }}
      ...
      <Modal
        ...
        width={700}
        open={visibleDetails}
        onCancel={() => setVisibleDetails(false)}
        title={"View Details"}
        footer={null}
      >
        {/* aaaa */}
