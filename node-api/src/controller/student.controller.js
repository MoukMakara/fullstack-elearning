...
const getList = async (req, res) => {
  const { page = 1, txtSearch, status, fromDate, toDate } = req.query;

  // Ensure page is a number and set default to 1 if invalid
  ...
  try {
    let sqlTotal = "SELECT COUNT(Id) AS totalRecords FROM student WHERE 1=1";
...
const create = async (req, res) => {
  try {
    ...
    const [findstudent] = await db.query(
      "SELECT * FROM student WHERE (Tel=:Tel OR Email=:Email)",
      param
    );
...
const studentRegisterGetList = async (req, res) => {
  try {
    ...
    const [classroom] = await db.query(
      "SELECT cr.*,c.Name as CourseName FROM classroom cr INNER JOIN course c on cr.CourseId = c.Id"
    );
...
const studentRegisterCreate = async (req, res) => {
  try {
    ...
    if (
      !isEmptyOrNull(Payment) &&
      !isEmptyOrNull(PaymentMethod) &&
      !isEmptyOrNull(PaymentDate)
    ) {
